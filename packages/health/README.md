...
- `serverExtend(read)` — memoise a `() => serverInfo | undefined` reader into an `extend` hook that renders `{ server }` once and `{}` off-platform; the shared shape behind `flyExtend()`, `koyebExtend()`, `railwayExtend()` and `vercelExtend()`. `OmitOptions<TInfo, K>` and `ServerEnvOptions<TInfo, K>` are the option types hosting packages build on.
