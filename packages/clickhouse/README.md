...
verifies your credentials and database — and fails the check when it comes
back unsuccessful.
...
that the credentials and the database are good — on ClickHouse Cloud, where
the default sends a real query, that can also wake an idle service.
...
of it. `@clickhouse/client-web` works the same way — it always sends the
