...
- `packages/<name>/` — one npm/JSR package per concern. `health` is the
  ...
  are server adapters; `bullmq`, `clerk`, `clickhouse`, `cloudflare-d1`,
  `cloudflare-kv`, `cloudflare-r2`, `drizzle`, `inngest`, `kafka`, `mongodb`,
  `mysql`, `nats`, `neon`, `planetscale`, `postgres`, `posthog`, `prisma`,
  `qstash`, `redis`, `resend`, `s3`, `sentry`, `stripe`, `supabase`,
  `tinybird`, `trigger-dev`, `turso`, `turso-serverless`, `unkey`, `upstash`,
  `workos` are probes; `fly`, `koyeb`, `railway`, `vercel`, `cloudflare`
  render hosting metadata under `server` via the `extend` hook.
